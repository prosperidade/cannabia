...
const FALLBACK_STATS: StatItem[] = [
  { icon: "science", label: "Ensaios Ativos", value: "—", delta: "—", deltaType: "up" },
  { icon: "groups", label: "Pacientes Participantes", value: "—", delta: "—", deltaType: "up" },
  { icon: "verified", label: "Taxa de Conclusao", value: "—", delta: "—", deltaType: "up" },
  { icon: "article", label: "Publicacoes", value: "—", delta: "—", deltaType: "up" },
];
...
export default function EnsaiosPage() {
  ...
  const [recruitment, setRecruitment] = useState<RecruitmentItem[]>([]);

  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const res = await getClinicalTrials();
      const d = res.data as Record<string, unknown>;

      if (Array.isArray(d.stats)) setStats(d.stats as StatItem[]);
      if (Array.isArray(d.trials)) setTrials(d.trials as Trial[]);
      if (Array.isArray(d.findings)) setFindings(d.findings as Finding[]);
      if (Array.isArray(d.eligibility)) setEligibility(d.eligibility as Eligibility[]);
      if (Array.isArray(d.recruitment)) setRecruitment(d.recruitment as RecruitmentItem[]);
    } catch {
      // keep fallback
    } finally {
      setLoading(false);
    ...
    return (
      <section className="p-4 md:p-8 flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-stone-400 text-sm">Carregando ensaios clinicos...</p>
        </div>
      </section>
    );
  }
        ...
        <div className="lg:col-span-2">
          <Card variant="glass" padding="sm" className="rounded-3xl overflow-hidden">
            <div className="p-4 md:p-6 border-b border-white/5 flex justify-between items-center">
              ...
              <Badge tone="primary">{trials.length} ativos</Badge>
            </div>
