...
| F3.3 | Captura estruturada de eventos adversos via WhatsApp/web/consulta — serviço `src/services/adverse_event_service.py` | F3.1 | Alta | Aberto |
| F3.4 | Extensão `src/ai/agents/regulatorio.py` — skill `triage_adverse_event(report)` com classificação de severidade | F3.1 | Alta | Aberto |
| F3.5 | Integração `src/integrations/vigimed.py` — notificação automatizada à ANVISA via VigiMed/Notivisa | F3.3 | Alta | Aberto |
| F3.6 | Blueprint `src/web/routes/pharmacovigilance.py` — captura, triagem, notificação e dashboard epidemiológico | F3.3, F3.5 | Alta | Aberto |
| F3.7 | Blueprint `src/web/routes/regulatory_reporting.py` — dashboards ANVISA-ready com indicadores calculados em tempo real | F3.2 | Alta | Aberto |
...
| ID | Tarefa | Dependência | Prioridade | Status |
|----|--------|-------------|-----------|--------|
| F4.1 | Serviço `src/services/evidence_service.py` — agregação longitudinal por condição clínica, correlação dose-efeito, extração de desfechos da telemetria pós-consulta (D+3/D+7/D+15 já existente) | F2.5 | Média | Aberto |
| F4.2 | Geração de estudos observacionais internos com metodologia reprodutível | F4.1 | Baixa | Aberto |
